...
const DatePicker = ({selectedDates, setSelectedDates}) => {
  ...
  return (
    <div className="date-picker">
      <head>
      <style>
        @import url('https://fonts.googleapis.com/css2?family=Alumni+Sans:wght@200;400&display=swap');
        </style>
      </head>
      <div className="input-wrapper">
        <input className="date-input" style={{overflow: "scroll"}}type="text" placeholder="Select event date(s)" readOnly value={selectedDates.map(d=> `${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}`).join(' - ')}/>
        <FaRegCalendar id="date-icon"/>
      </div>
      <div className='date-body'>
        <div className="navigation">
          <IoChevronBackSharp className="navigate backward"onClick={() => navigateMonth('backward')}/>
          <span>{getSelectedMonthName() + " " + selectedMonth.getFullYear()}</span>
          <IoChevronForwardSharp className="navigate forward" onClick={() => navigateMonth('forward')}/>
        </div>
        ...
        {[ 'Su','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(day => (
          ...
          {getMonthArray(selectedMonth.getMonth()+1, selectedMonth.getFullYear()).map((day, index) => (
          day === " "  ?
          <button disabled>
          </button>
          :<button
              key={index}
              className={`date ${selectedDates.map(date => date.getTime()).includes(day.getTime()) ? 'selected' : ''}`}

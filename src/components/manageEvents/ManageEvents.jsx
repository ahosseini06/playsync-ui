...
const ManageEvents = ({onView, onEdit, onRemove}) => {
  ...
  return (
    <>
     {/*modal*/}
     <Modal colorScheme='whatsapp'isCentered motionPreset='slideInRight' closeOnOverlayClick={false} isOpen={isOpen} size={"xxl"} onClose={onClose}  scrollBehavior='outside'>
        <ModalOverlay />
        <ModalContent style={{backgroundColor: "#D1C4E9", width:"800px", borderRadius:'25px'}}>
          <ModalHeader className="modal-header"><div className='modal-title'>Create New Event</div>
          ...
          <ModalFooter>
            <div className="m-footer">
              <button className="btn-secondary" onClick={stage===1? () =>{ onClose(); setStage(1)} : ()=> {setStage(stage-1)}}>
              {stage === 1 ? "Cancel" : "Back"}
              </button>
                  ...
                  <Popover>
                    <PopoverTrigger>
                      <Button className="btn-primary" onClick={stage === 4 ? createTournament : nextStage } >
                    ...
                    <PopoverContent  color='#E040FB' bg='#1E113C' borderColor='#1E113C'>
                      <PopoverCloseButton />
                      <PopoverHeader borderColor='black'>Please fill out all required fields</PopoverHeader>
                    </PopoverContent>
                    }
                  </Popover>
